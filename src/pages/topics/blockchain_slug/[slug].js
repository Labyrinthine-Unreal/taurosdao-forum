// src/pages/topics/blockchain_slug/[slug].js
...
import ReplyButton from '@root/components/buttons/blockchainReplyButton';
...
const TopicPage = () => {
  ...
  const topicData = data?.blockchain_by_slug.slug;
  ...
  const isAuthor = user.username === data?.blockchain_by_slug.user // check if current user is the author

    return (
    <div>
      <Header/>
      <div className={styles.topicHeading}>{data?.blockchain_by_slug.topic}</div>
      <div className={styles.container}>
        <div className={styles.tableContainer}>
            ...
            <tbody>
              <tr>
                <td className={styles.leftColumn}>
                  <div>{topicData.user}
                  {data?.blockchain_by_slug.eth_address}
                ...
                <div className={styles.titleBlock}>
                  <div className={styles.title}>{data?.blockchain_by_slug.topic}</div>
                    <p className={styles.date}>19 February 2023, 22:09</p>
                  </div>
                  <div className={styles.content}>{parse(data?.blockchain_by_slug.content)}</div>
                  ...
                  {isAuthor && 
                    ...
                    >
                      <div>
                        {isAuthor && showEdit &&
                          <UpdateTopic category="blockchain" setShowEdit={setShowEdit} />}
          ...
          <ReplyButton />
          <GPT />
          <CommentList category="blockchain" />
        </div>
      </div>
    </div>
  );
