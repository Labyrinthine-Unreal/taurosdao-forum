...
import styles from '@root/pages/TopicPage.module.css';
...
const TopicPage = () => {
  ...
  const isAuthor = user.username === data?.art_by_slug.user // check if current user is the author
    ...
    <div>
      <Header/>
      <div className={styles.topicHeading}>{data?.art_by_slug.topic}</div>
            ...
            <tbody>
              <tr>
                ...
                <div className={styles.title}>{data?.art_by_slug.eth_address}</div>
                  ...
                  <div className={styles.content}>{parse(data?.art_by_slug.content)}</div>
