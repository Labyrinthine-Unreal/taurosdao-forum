// src/pages/topics/game_slug/[slug].js
...
const TopicPage = () => {
  ...
  const isAuthor = user.username === data?.game_by_slug.user // check if current user is the author

    return (
    <div>
      <Header/>
      <div className={styles.topicHeading}>{data?.game_by_slug.topic}</div>
      <div className={styles.container}>
        <div className={styles.tableContainer}>
            ...
            <tbody>
              <tr>
                ...
                <div className={styles.titleBlock}>
                  <div className={styles.title}>{data?.game_by_slug.topic}</div>
                    <p className={styles.date}>19 February 2023, 22:09</p>
                  </div>
                  <div className={styles.content}>{parse(data?.game_by_slug.content)}</div>
                  ...
                  {isAuthor && 
                    ...
                    >
                      <div>
                        {isAuthor && showEdit &&
                          <UpdateTopic category="game" setShowEdit={setShowEdit} />}
          ...
          <ReplyButton category="game" />
          <GPT />
          <CommentList category="game" />
        </div>
      </div>
    </div>
  );
